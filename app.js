...
function renderDash(){
  ...
  w.innerHTML='<div class="grid grid-cols-12 gap-5">'+
    ...
    '<section class="hidden col-span-12 rounded-2xl border border-white/10 bg-gradient-to-br from-primaryContainer/20 to-surfaceLow p-5 shadow-xl backdrop-blur-xl md:block lg:col-span-6"><div><span class="font-inter text-xs font-extrabold uppercase tracking-[.16em] text-textMuted">'+t('monthlyNetBalance')+'</span><strong class="mt-3 block font-spaceGrotesk text-4xl font-bold text-textMain md:text-5xl">฿ '+fmt2(net)+'</strong><small class="mt-4 flex items-center gap-2 text-sm font-bold '+(net>=0?'text-green':'text-danger')+'"><span class="material-symbols-outlined text-lg">trending_up</span> '+t('incomeLabel')+' ฿ '+fmt(incMo)+' · '+t('expenseLabel')+' ฿ '+fmt(tot)+'</small></div></section>'+
